...
Zotero.Integration = new function() {
	...
	// Needed to create temp files to write citations to text document
...
/**
 * Added For Zotero PowerPoint
 */

	/**
	 * The data that would normally be returned by the word processor, but couldn't be returned this way by PowerPoint
	 * This data might need adjustment
	 */
	function pptData(citationStyle) {
		var data = {
		    "style": {
		...
		data.style.styleID = citationStyle;
		return data;
	}


	/**
	* This will act as a 'document' so some of the functions that need a document become usable
	**/
	function pptDoc() {

		// This function is needed to substitute the cleanup function used at the displayDialog function
	...
	/**
	* This will act as the io variable which is used in quickFormat.js
	**/
	function pptIo(citationStyle) {
		...
		this.promise = me._acceptDeferred.promise.then(function(progressCallback) {
			var item = new Zotero.Item;
			item = Zotero.Items.get(me.citation.citationItems[0].id);

			//return all available citation styles and styleID's
			...
			var data = pptData(citationStyle);
			var getStyle = Zotero.Styles.get(data.style.styleID);
			data.style.hasBibliography = getStyle.hasBibliography;
			me.style = getStyle.getCiteProc(data.prefs.automaticJournalAbbreviations);
			me.style.setOutputFormat("rtf");
			me.styleClass = getStyle.class;

			var citationText = "";

			// When there are multiple citations in the searchbox, loop through them
			for (var i=0; i < me.citation.citationItems.length; i++) {

				// set a static citation, only id will be used anyways
				var testCitation = {
					"citationItems": [{"id": 11}],
				...
				// Get the citation strings in the correct format
				if(citationStyle == "http://www.zotero.org/styles/ieee" || citationStyle == "http://www.zotero.org/styles/vancouver" || citationStyle == "http://www.zotero.org/styles/cell" || citationStyle == "http://www.zotero.org/styles/nature") {
					var my_ids = [testCitation.citationItems[0].id];
					me.style.updateItems( my_ids, true );
					var bib = me.style.makeBibliography();
					// Delete the first part of the citation and add delimiter #cit#
					citationText += (bib[1] + "").split("tab ").pop() + "#cit#";
					...
					for each(var newCitation in newCitations[1]) {
						citationText += newCitation[1] + "#cit#";
					}
				}
			}
			
			// create the temporary file with the citations
			var file = FileUtils.getFile("TmpD", ["zoteroCitation.tmp"]);
			try {
				file.create(Components.interfaces.nsIFile.NORMAL_FILE_TYPE, FileUtils.PERMS_FILE);
			}
			catch (err) {
				alert("something went wrong when creating temp file: " + err);
			}
			...
			var istream = converter.convertToInputStream(citationText);
		...
		// Return emty array of items, used at _quickformat function in quickFormat.js
		...
		// Pretend that the session has been updated
		this._updateSession = function updateSession() {
			return Q.resolve(true);
		}

		// Substitute for the accept function, called at _accept() in quickFormat.js
	...
	/**
	* The command function used when trying to use some of the search functionality of the intergration.js file
	*
	**/
	this.pptCommand = new function() {
		var inProgress;

		return function pptCommand(citationStyle) {

			var field = "field";

			return Q(field).then(function(field) {
				// Create the needed substitute variables
				var doc = new pptDoc();
				var io = new pptIo(citationStyle);
				
				// Classic citation dialog not implemented here, only the quickformat
				var mode = (!Zotero.isMac && Zotero.Prefs.get('integration.keepAddCitationDialogRaised')
					? 'popup' : 'alwaysRaised')
				Zotero.Integration.displayDialog(doc,
				'chrome://zotero/content/integration/quickFormatPpt.xul', mode, io);
...
/**
 * End of additions for Zotero Powerpoint
 */
...
Zotero.Integration.CorruptBibliographyException = function(code, cause) {
	alert("CorruptBibliographyException");
	this.code = code;
	this.cause = cause;
}
...
Zotero.Integration.Presentation = function(app, pres) {
	alert("Presentation");
	this._app = app;
	this._pres = pres;
}
...
Zotero.Integration.Fields.prototype._processFields = function(i) {
	...
	for(var n = this._fields.length; i<n; i++) {
		...
		alert("content: " + JSON.stringify(content, null, 4));
		if(type === INTEGRATION_TYPE_ITEM) {
			...
			if(this.ignoreEmptyBibliography && field.getText().trim() === "") {
				...
				alert("pushed: " + JSON.stringify(this._bibliographyFields, null, 4));
...
Zotero.Integration.Fields.prototype._updateDocument = function(forceCitations, forceBibliography,
	...
	if(this._bibliographyFields.length	 				// if bibliography exists
		...
		if(forceBibliography || this._session.bibliographyDataHasChanged) {
			...
			alert("bibliographydata: " + JSON.stringify(bibliographyData, null, 4));
		...
		alert("textconstruct: " + JSON.stringify(bib, null, 4));
...
Zotero.Integration.Session.prototype.addCitation = function(index, noteIndex, arg) {
	...
	alert("returned citation: " + JSON.stringify(citation, null, 4));
	// get items
	...
	alert("after lookup: " + JSON.stringify(citation, null, 4));
...
Zotero.Integration.Session.prototype.lookupItems = function(citation, index) {
	alert("lookupItems");
	alert("lookupitems: " + JSON.stringify(this.citationsByIndex, null, 4));
	for(var i=0, n=citation.citationItems.length; i<n; i++) {
		...
		if(zoteroItem) {
			alert(JSON.stringify(zoteroItem, null, 4));
...
Zotero.Integration.Session.prototype.getBibliography = function() {
	...
	alert("bib: " + JSON.stringify(bib, null, 4));
	if(bib) {
		// omit items
...
Zotero.Integration.Session.prototype.updateUncitedItems = function() {
	...
	alert("unciteditems length: " + this.uncitedItems.length);
	alert("unciteditems: " + JSON.stringify(this.uncitedItems));
...
Zotero.Integration.Session.prototype.formatCitation = function(index, citation) {
	...
	if(!this.citationText[index]) {
		...
		alert(JSON.stringify(newCitations, null, 4));
		for each(var newCitation in newCitations[1]) {
		...
		alert("newCitations: " + JSON.stringify(this.newCitations, null, 4));
	}
}
...
Zotero.Integration.Session.prototype._updateCitations = function() {
	...
	for each(var indexList in [this.newIndices, this.updateIndices]) {
		...
		for(var index in indexList) {
			...
			alert("the citation is now1: " + JSON.stringify(citation, null, 4));
			if(!citation || citation.properties.delete) continue;
			if(this.formatCitation(index, citation)) {
				this.bibliographyHasChanged = true;
			}
			alert("the citation is now2: " + JSON.stringify(citation, null, 4));
...
Zotero.Integration.Session.prototype.getBibliographyData = function() {
	alert("getBibliographyData");
	var bibliographyData = {};
	
	alert("uncited items: " + JSON.stringify(this.omittedItems, null, 4));
...
Zotero.Integration.Session.BibliographyEditInterface = function(session) {
	alert("BibliographyEditInterface");
...
Zotero.Integration.DocumentData = function(string) {
	...
	this.prefs = {};
	this.sessionID = null;
	alert("documentdatastring: " + string);
	if(string) {
